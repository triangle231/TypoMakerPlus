        ...
        if (this.dragState.isDraggingKeyframe) {
            ...
            if (this.dragState.isDraggingValue && this.dragState.keyframeProperty) {
                ...
                if (track) {
                    ...
                    // 트랙 범위를 벗어나도 값이 튀지 않도록 clamp
                    const clampedY = Math.max(padding, Math.min(trackHeight - padding, currentY));

                    // Y 좌표를 값으로 변환 후 0~1 범위 제한
                    let normalizedY = (graphHeight - (clampedY - padding)) / graphHeight;
                    ...
                    // 값 범위를 최소/최대에 맞게 clamp
                    roundedValue = Math.max(this.dragState.keyframeMinValue, Math.min(this.dragState.keyframeMaxValue, roundedValue));
