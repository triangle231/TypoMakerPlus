...
class AnimationGraph {
    ...
    handleMouseUp(e) {
        ...
        // 드래그하지 않고 클릭만 했으면 - 클릭한 위치로 값 설정
        if (!this.hasMoved && clip && clip.animation && clip.animation[this.property]) {
            // 클릭한 위치 계산
            const rect = this.canvas.getBoundingClientRect();
            ...
            // 드래그했으면 값이 변경되었는지 확인
            const anim = clip.animation[this.property];
            const currentValue = this.dragTarget === 'start' ? anim.start : anim.end;
            if (this.initialValue !== undefined && this.initialValue !== currentValue) {
                valueChanged = true;
            }
        }
