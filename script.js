        ...
        try {
            ...
            function renderClipToImage(clip) {
                return new Promise((resolve) => {
                    ...
                    const minSize = 100;
                    const padding = clip.type === 'text' ? 20 : 7.5; // 도형은 여백 없이 크기 그대로
                    // 화질 향상을 위해 캔버스 크기를 배율 적용
                ...
                if (clip.type === 'image') {
                    ...
                    promise = new Promise((resolve) => {
                        ...
                        img.onload = function () {
                            ...
                            // 이미지는 원본 크기 그대로 사용 (배율 1)
                            const qualityScale = 1;
                            const imgWidth = img.naturalWidth;
                            const imgHeight = img.naturalHeight;
                            
                            // 캔버스 크기를 이미지 크기로 설정
                            canvas.width = imgWidth * qualityScale;
                            canvas.height = imgHeight * qualityScale;
                        ...
                        if (clip.imageURL) {
                            ...
                            if (clip.imageFile instanceof Blob) {
                                img.src = URL.createObjectURL(clip.imageFile);
                            } else if (typeof clip.imageFile === 'string') {
                                img.src = clip.imageFile;
                            }
                        }
                    });
                } else {
    ...
    function loadProject(projectData) {
        ...
        Object.values(clipsMap).forEach(clipData => {
            ...
            if (clipData.type === 'image' && clipData.imageData) {
                ...
                // imageFile도 data URL로 설정 (내보내기 시 필요)
                clipData.imageFile = clipData.imageData;
